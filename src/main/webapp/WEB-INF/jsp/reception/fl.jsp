            ...
            <div class="layui-input-group">
              <div class="layui-input-block" style="width: 100px">
                <select name="interest" lay-filter="fltype">
                  <option value="0" selected>认领</option>
...
<%--                <button class="layui-btn">搜索</button>--%>
                <a href="sfl?id=0" class="layui-btn">发布认领</a>
                <a href="sfl?id=1" class="layui-btn">发布丢失</a>
              </div>
            </div>
          </div>
          <div class="layui-fluid" id="LAY-component-grid-list">
            <div id="itemList" class="layui-row layui-col-space10 demo-list">
              <c:if test="${not empty find}">
                <c:forEach items="${find}" var="fd">
                  <div class="layui-col-sm4 layui-col-md3 layui-col-lg3">
                    <!-- 填充内容 -->
                    <a href="detail?id=${fd.id}">
                      <div class="layui-card">
                        <img src="upload/${fd.picture}" alt=${fd.id}>
                      </div>
                    </a>
                  </div>
                </c:forEach>
          ...
          form.on('select(fltype)', function (data) {
            ...
            $.ajax({
              ...
              success: function (response) {
                ...
                $('#itemList').empty()
                $.each(jsonObject.list, function (index, ls) {
                  $('#itemList').append(
                    '<div class="layui-col-sm4 layui-col-md3 layui-col-lg3">' +
